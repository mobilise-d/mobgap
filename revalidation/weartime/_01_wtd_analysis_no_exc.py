...
It intentionally reads only local files generated by ``revalidation/weartime/_02_wtd_result_generation_no_exc.py``.

.. note:: If you are interested in how these results are calculated, head over to the processing script
    revalidation/weartime/_02_wtd_result_generation_no_exc.py. These results are not part of the published
    validation-result package at this time.
...
# algorithms can be added here once they are implemented as ``BaseWeartimeDetector`` instances and evaluated with the
# wear-time result-generation script.
