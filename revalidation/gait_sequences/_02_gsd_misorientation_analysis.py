...
This analysis is more of a confirmation which algorithms work even when the sensor is mounted incorrectly and which
mounting orientations are more problematic for the algorithms.
...
# Conclusion
# ----------
# As expected GSDionescu works identically across all orientations, as it relies on the norm of the accelerometer
# signal.
# GSDIluz is heavily depended on the correct orientation.
# If the gravity direction is on the wrong axis, the algorithm fails to find any gait sequence (with very few
# exceptions).
# Flipping the sensor front-back (pa_flipped__rot_pa_0) is less problematic and seems to result in only a small drop
# in performance for GSDIluz.
# But as expected, it makes no sense to use GSDIluz if the sensor is expected to be mounted in a different orientation
# than the default Mobilise-D one.
