...
def pairwise_tests(df: pd.DataFrame, dv: str, between: str) -> tuple[float, float]:
    result= pg.pairwise_tests(data=df, dv=dv, between=between)
    assert len(result) == 1
    return tuple(result.iloc[0][["T", "p-unc"]].to_list())

def agg_errors(df: pd.DataFrame, groupby: list[str], stats_between="version") -> pd.DataFrame:
    error_agg = df.groupby([*groupby, stats_between]).apply(apply_aggregations, custom_aggs, include_groups=False)
    stats = df.groupby(groupby).apply(apply_aggregations, [CustomOperation(identifier=None,
                                                                           function=partial(pairwise_tests, dv="wb__abs_error", between="version"),
                                                                           column_name = [("T", "wb__abs_error", ), ("p", "wb__abs_error")])], include_groups=False)
