...
# %%
# Comparing CNN and CNN-LSTM variants
# -----------------------------------
# The algorithm provides two model variants: baseline CNN and CNN-LSTM hybrid.

wtd_results = {}

for version in ["cnn", "cnn_lstm"]:
    print(f"\n{'=' * 60}")
    ...
    wtd_results[version] = wtd_output
...
# --------------------
# We can visualize both variants overlaid on the IMU signal.

fig, ax = plot_wtd_outputs(
    imu_data,
    cnn=wtd_results["cnn"].weartime_list_,
    cnn_lstm=wtd_results["cnn_lstm"].weartime_list_,
)
plt.tight_layout()
plt.show()
