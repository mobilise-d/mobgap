...
def extract_features_95pct(
    ...
    Computes an intermediate feature subset selected via SHAP analysis to explain
    95% of model predictions, balancing computational efficiency and predictive power.
    ...
    # Additional accelerometer IS features
    features["acc_is_rms"] = rms(acc_data[:, 0])
    jerk_is = np.diff(acc_data[:, 0]) / dt
    features["acc_is_jerk_rms"] = rms(jerk_is)
    features["acc_is_jerk_peak"] = np.max(np.abs(jerk_is))
    ...
    # Additional accelerometer PA features
    jerk_pa = np.diff(acc_data[:, 2]) / dt
    features["acc_pa_jerk_mean"] = np.mean(np.abs(jerk_pa))
    ...
    # Additional gyroscope ML features
