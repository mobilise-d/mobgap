...
## Dataset-backed TPCP workflows

The current CNN and XGBoost workflows train from raw SUSTAIN CWA recordings through
`SustainWearTimeDataset` and `WtdEmulationPipeline.self_optimize`. Their daily,
participant-grouped evaluations use TPCP `Optimize` and `DatasetSplitter`:

```sh
python scripts/weartime_ml_training_evaluation/training/train_cnn.py --help
python scripts/weartime_ml_training_evaluation/training/train_xgboost.py --help
python scripts/weartime_ml_training_evaluation/evaluation/loso_daily_cnn.py --help
python scripts/weartime_ml_training_evaluation/evaluation/loso_daily_xgboost.py --help
```

Install the `weartime` extra and supply `--dataset-path` or
`MOBGAP_SUSTAIN_WEARTIME_DATASET_PATH`. The remaining scripts below preserve the
original preprocessed-window training and evaluation workflow.

CNN training saves a `.keras` artifact. Load it in another Python process with
