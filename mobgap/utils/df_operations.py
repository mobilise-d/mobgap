...
def cut_into_overlapping_bins(df: pd.DataFrame, column: str, interval_dict: dict[str, tuple[float, float]]) -> pd.DataFrame:
    """
    ...
    groups = {
        key: df[
            (col_of_interest > val[0]) & (col_of_interest <= val[1])]
        for key, val in interval_dict.items()
    }

    return pd.concat(groups, names=["bin", *df.index.names]).reset_index("bin").astype({"bin": pd.CategoricalDtype(list(interval_dict.keys()), ordered=True)})


__all__ = ["MultiGroupBy", "create_multi_groupby", "CustomOperation", "apply_transformations", "apply_aggregations", "cut_into_overlapping_bins"]
