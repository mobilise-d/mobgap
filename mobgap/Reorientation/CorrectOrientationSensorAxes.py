    ...
    if n_sgfilt < len(accx):
        ...
        if n > 2:
            for GS, data_slice in iter_gs(av_filt1, gs):
                ...
                if avm >= th:
                    corIMUdata.iloc[GS.start:GS.end, 0:3] = acc.iloc[GS.start:GS.end, :]
                elif avm <= -th:
                    corIMUdata.iloc[GS.start:GS.end, 0] = -acc.iloc[GS.start:GS.end, 0]
                    corIMUdataSequence.loc[k, ['Start', 'End']] = [GS.start, GS.end]
                    k = k + 1
                else:
                    corIMUdata.iloc[GS.start:GS.end, :] = CorrectSensorOrientationDynamic(data.iloc[GS.start:GS.end, :], sampling_rate_hz)
                    corIMUdataSequence.loc[k, ['Start', 'End']] = [GS.start, GS.end]
                    k = k + 1
